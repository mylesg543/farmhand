...
export function PnLPage() {
  ...
  // Per-animal P&L — always positive income, always positive costs, net = income - costs
  const animalPnL = {}
  ...
  const handleAddIncome = async () => {
    ...
    try {
      await addIncome({ species: incomeForm.species, income_type: incomeForm.income_type, description: incomeForm.description, amount: amt, date: incomeForm.date })
      setIncomeForm({ species: 'sheep', income_type: 'sale_animal', description: '', amount: '', date: today })
  ...
  return (
    <div style={S.page}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', marginBottom: 24 }}>
        <div>
          <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: 32, fontWeight: 700, margin: '0 0 4px' }}>Profit & Loss</h1>
          <p style={{ fontSize: 14, color: '#a08060', margin: 0 }}>Track what you spend and what you earn</p>
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 10 }}>
          ...
          <button onClick={() => { setShowCostForm(v => !v); setShowIncomeForm(false) }}
            ...
            {showCostForm ? '✕ Cancel' : '+ Log Expense'}
          </button>
        </div>
      </div>
      ...
      {/* Income form */}
      {showIncomeForm && (
        <div style={{ ...S.card, padding: 24, marginBottom: 20, border: '1px dashed #a5d6a7', background: '#f1f8f1' }}>
          <span style={{ ...S.sectionLabel, color: '#2e7d32' }}>Log Income — Money Coming In</span>
          {incomeErr && <p style={{ color: '#c62828', fontSize: 13, marginBottom: 10 }}>{incomeErr}</p>}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 2fr 1fr', gap: 14, marginBottom: 14 }}>
            <div>
            ...
            <div>
              ...
              <input style={S.input} value={incomeForm.description} onChange={e => setI('description', e.target.value)} placeholder="e.g. Sold 3 lambs to Jones Farm" />
            </div>
            <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 14, alignItems: 'flex-end' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={handleAddIncome} style={{ ...S.btn, background: '#4caf50', color: '#fff' }}>Save Income</button>
      ...
      {showCostForm && (
        <div style={{ ...S.card, padding: 24, marginBottom: 20, border: '1px dashed #c8b89a', background: '#fdfaf6' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '160px 1fr', gap: 14, alignItems: 'flex-end' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={handleAddCost} style={{ ...S.btn, ...S.btnPrimary }}>Save Expense</button>
      ...
      {/* Summary tiles — income always green, expense always red, net = profit or loss */}
        ...
        {[{ key: 'all', label: 'All Animals', emoji: '🌾' }, ...Object.entries(ANIMAL_META).map(([k, v]) => ({ key: k, label: v.label, emoji: v.emoji }))].map(a => {
          ...
          return (
            <div key={a.key} onClick={() => setFilterAnimal(a.key)}
              style={{ ...S.card, padding: '14px 12px', cursor: 'pointer', textAlign: 'center', border: isActive ? `2px solid ${netColor(p.net)}` : '1px solid #e8e0d0', background: isActive ? netBg(p.net) : '#fff', transition: 'all 0.15s' }}>
              <div style={{ fontSize: 22, marginBottom: 4 }}>{a.emoji}</div>
              <div style={{ fontSize: 10, color: '#a08060', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6 }}>{a.label}</div>
              {/* Net P&L */}
              <div style={{ fontSize: 16, fontWeight: 700, color: netColor(p.net), fontFamily: "'Playfair Display',serif" }}>{fmtNet(p.net)}</div>
              {/* Income always green + , Expenses always red - */}
              <div style={{ fontSize: 10, color: '#a08060', marginTop: 4, display: 'flex', justifyContent: 'center', gap: 6 }}>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20, alignItems: 'center' }}>
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 20, alignItems: 'center' }}>
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: 10, color: '#2e7d32', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 1px' }}>Income</p>
            <p style={{ fontSize: 18, fontWeight: 700, color: '#2e7d32', margin: 0 }}>+{fmt(totalIncome)}</p>
          </div>
          <div style={{ textAlign: 'center' }}>
            <p style={{ fontSize: 10, color: '#c62828', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 1px' }}>Expenses</p>
            <p style={{ fontSize: 18, fontWeight: 700, color: '#c62828', margin: 0 }}>-{fmt(totalCosts)}</p>
          </div>
          <div style={{ textAlign: 'center', borderLeft: '1px solid #e8e0d0', paddingLeft: 20 }}>
            <p style={{ fontSize: 10, color: '#a08060', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', margin: '0 0 1px' }}>Net P&L</p>
            <p style={{ fontSize: 22, fontWeight: 700, color: netColor(netPnL), fontFamily: "'Playfair Display',serif", margin: 0 }}>{fmtNet(netPnL)}</p>
          </div>
        </div>
      </div>
      ...
      {loading ? <Spinner /> : error ? <ErrorMsg message={error} /> : (
        <>
          ...
          {tab === 'overview' && (
            months.length === 0
              ? <div style={{ ...S.card, padding: 60, textAlign: 'center' }}><p style={{ color: '#a08060', fontSize: 15 }}>No entries yet. Log your first income or expense above.</p></div>
              : months.map(mo => {
                    ...
                    <div key={mo} style={{ ...S.card, padding: 22, marginBottom: 14 }}>
                      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 14, paddingBottom: 10, borderBottom: '1px solid #f0ebe4' }}>
                        ...
                        <div style={{ marginLeft: 'auto', display: 'flex', gap: 16, alignItems: 'center' }}>
                          <span style={{ fontSize: 13, color: '#2e7d32', fontWeight: 600 }}>+{fmt(moEarned)}</span>
                          <span style={{ fontSize: 13, color: '#c62828', fontWeight: 600 }}>-{fmt(moSpent)}</span>
                          <span style={{ fontSize: 15, fontWeight: 700, color: netColor(moNet), borderLeft: '1px solid #e8e0d0', paddingLeft: 12 }}>{fmtNet(moNet)}</span>
                        </div>
                      </div>
                          ...
                          .map(entry => {
                            ...
                            return (
                              <div key={entry.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '9px 12px', borderRadius: 8, background: isIncome ? '#f1f8f1' : '#fdfaf6' }}>
                                ...
                                <span style={{ fontWeight: 700, fontSize: 14, color: isIncome ? '#2e7d32' : '#c62828', flexShrink: 0 }}>
          ...
          {/* Income tab — all green, all positive */}
          {tab === 'income' && (
            filteredIncome.length === 0
              ? <div style={{ ...S.card, padding: 60, textAlign: 'center' }}><p style={{ color: '#a08060' }}>No income recorded yet.</p></div>
              : months.filter(mo => filteredIncome.some(i => i.date.startsWith(mo))).map(mo => {
                  ...
                  return (
                    <div key={mo} style={{ ...S.card, padding: 22, marginBottom: 14 }}>
                      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 14, paddingBottom: 10, borderBottom: '1px solid #f0ebe4' }}>
                        <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 16, fontWeight: 700 }}>{fmtMonth(mo)}</span>
                        <span style={{ marginLeft: 'auto', fontWeight: 700, fontSize: 15, color: '#2e7d32' }}>+{fmt(total)}</span>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {entries.map(i => {
                          ...
                          return (
                            <div key={i.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 8, background: '#f1f8f1' }}>
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <p style={{ fontSize: 11, color: '#a08060', margin: 0 }}>{meta.label} · {iType} · {formatDate(i.date)}</p>
                              </div>
          ...
          {/* Expenses tab — all red, all negative */}
          {tab === 'expenses' && (
            filteredCosts.length === 0
              ? <div style={{ ...S.card, padding: 60, textAlign: 'center' }}><p style={{ color: '#a08060' }}>No expenses recorded yet.</p></div>
              : months.filter(mo => filteredCosts.some(c => c.date.startsWith(mo))).map(mo => {
                  ...
                  return (
                    <div key={mo} style={{ ...S.card, padding: 22, marginBottom: 14 }}>
                      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 14, paddingBottom: 10, borderBottom: '1px solid #f0ebe4' }}>
                        <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 16, fontWeight: 700 }}>{fmtMonth(mo)}</span>
                        <span style={{ marginLeft: 'auto', fontWeight: 700, fontSize: 15, color: '#c62828' }}>-{fmt(total)}</span>
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {entries.map(c => {
