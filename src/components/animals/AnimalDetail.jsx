...
function MiniLineage({ animal, allAnimals, navigate, isMobile }) {
  ...
  return (
    <div style={{ ...S.card, padding:isMobile?'14px 16px':'18px 22px', marginBottom:14 }}>
      <div style={{ display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', marginBottom:hasParents?14:(isMobile?16:0) }}>
      ...
      {!hasParents ? (
        <div style={{ display:'flex', alignItems:isMobile?'stretch':'center', gap:isMobile?8:12, flexWrap:isMobile?'wrap':'nowrap', flexDirection:isMobile?'column':'row' }}>
          <p style={{ fontSize:13, color:'#a08060', margin:0, flex:1 }}>No parents recorded yet.</p>
          <button onClick={()=>navigate(`/animals/${animal.id}/edit`)}
            style={{ ...S.btn, ...S.btnSecondary, justifyContent:isMobile?'center':undefined, padding:isMobile?'8px 12px':'5px 12px', fontSize:12, minHeight:isMobile?38:undefined }}>+ Add Parents</button>
        </div>
      ) : (
